...
if [[ $1 == "-h" || $1 == "" ]]; then
    echo "Usage: get_SeqReads [-F|-P] [USER_ORGANISM | \"USR_Genus USR_species\] [WGS|WXS|AMPLICON|RNA-Seq|RAD-Seq|ChIP-Seq|Hi-C]"
    echo "Enter search query organism for lists of sequence run archive information; If entering name with more than one word, use quotes"
